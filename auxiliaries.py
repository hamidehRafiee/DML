...
def eval_metrics_one_dataset(model, test_dataloader, device, k_vals, opt,LOG):
    """
    ...
    with torch.no_grad():
        ...
        for idx, inp in enumerate(final_iter):
            ...
            # --------------
            feats = out[0].view(out[0].size(0), -1)
            embeddings.append(feats.cpu().numpy())
            labels.append(target.cpu().numpy())
            # --------------

            feature_coll.extend(out[0].cpu().detach().numpy().tolist())
        embeddings = np.concatenate(embeddings, axis=0)
        labels = np.concatenate(labels, axis=0)

        np.save(LOG.prop.save_path +'embeddings.npy', embeddings)
        np.save(LOG.prop.save_path +'labels.npy', labels)
...
def metrics_to_examine(dataset, k_vals):
    ...
    """
    metric_dict = {'train': ['Epochs', 'Time', 'Train Loss', "triplet on real", "triplet on cluster","cross entropy on real"]}
