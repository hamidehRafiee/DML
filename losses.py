...
def loss_select(loss, opt, to_optim):
    ...
    elif loss == "clustertriplet":
        loss_params = {'margin': opt.margin, 'sampling_method': "softhard"}#softhard, opt.sampling
        criterion = TripletLossCluster(**loss_params)

    elif loss == "triplet_new_sampling":
        loss_params = {'margin': opt.margin, 'sampling_method': "mysampling"}#softhard, opt.sampling
        criterion = TripletLoss_new_sampling(**loss_params)
        print(" TripletLoss_new_sampling loss set with my sampling ...")


    elif loss == "margin_new_sampling":
        loss_params = {'margin': opt.margin, 'sampling_method': "mysampling"}#softhard, opt.sampling
        criterion = MarginLoss_M(**loss_params)
        print("MarginLoss_M loss set with distance_m sampling...")
    ...
    elif loss == 'clustermarginloss':
        loss_params = {'margin': opt.margin, 'nu': opt.nu, 'beta': opt.beta,
                       'n_classes': opt.num_classes, 'sampling_method': opt.sampling}
        # 'random, semihard, distance, softhard.'
        criterion = MarginLossCluster(**loss_params)
        to_optim += [{'params': criterion.parameters(), 'lr': opt.beta_lr, 'weight_decay': 0}]


    elif loss == 'proxynca':
        loss_params = {'num_proxies': opt.num_classes,
                       'embedding_dim': opt.classembed if 'num_cluster' in vars(
                           opt).keys() else opt.embed_dim}
...
class TupleSampler():
    ...
    def mysampling(self, batch, labels, pred_labels):
        if isinstance(labels, torch.Tensor):
            labels = labels.detach().numpy()
        bs = batch.size(0)
        ...
        for i in range(bs):
            n = None
            if (labels[i] != pred_labels[0][i]) and (pred_labels[0][i] in labels):
                n = 1
            elif (labels[i] != pred_labels[1][i]) and (pred_labels[1][i] in labels):
                n = 2

            elif (labels[i] != pred_labels[2][i]) and (pred_labels[2][i] in labels):
                n = 3
            ...
            if n:
                ...
                anch = labels == pred_labels[n-1][i]
                # find count of True in anch vector
                negatives.append(i)

                anch_mask = np.logical_and(anch, d>=d[np.where(anch)[0]].max())
                if anch_mask.sum() > 0:
                    anchors.append(np.where(anch_mask)[0][0])
                else:
                    anchors.append(np.random.choice(np.where(anch)[0][0]))

                #######################

                anch[np.where(anch_mask)[0]] = False
                pos_mask = np.logical_and(anch, d>=d[np.where(anch)[0]].max())
                if pos_mask.sum() > 0:
                    positives.append(np.where(pos_mask)[0][0])
                else:
                    positives.append(np.random.choice(np.where(anch)[0][0]))
    ...
    def distanceweightedsampling_M(self, batch, labels, pred_labels,lower_cutoff=0.5, upper_cutoff=1.4):
        ...
        for i in range(bs):
            ...
            if n:
                ...
                l, d = labels[i], distances[i].detach().cpu().numpy()
                ...
                anch_mask = np.logical_and(anch, d >= d[np.where(anch)[0]].max())
                if anch_mask.sum() > 0:
                    anchors.append(np.where(anch_mask)[0])
                else:
                    anchors.append(np.random.choice(np.where(anch)[0]))
                ...
                pos_mask = np.logical_and(anch, d >= d[np.where(anch)[0]].max())
                if pos_mask.sum() > 0:
                    positives.append(np.where(pos_mask)[0])
                else:
                    positives.append(np.random.choice(np.where(anch)[0]))
...
### Standard Triplet Loss, finds triplets in Mini-batches.
class TripletLoss_new_sampling(torch.nn.Module):
    def __init__(self, margin=1, sampling_method='random'):
        """
        Basic Triplet Loss as proposed in 'FaceNet: A Unified Embedding for Face Recognition and Clustering'
        Args:
            ...
            sampling_method:    Method to use for sampling training triplets. Used for the TupleSampler-class.
        """
        super(TripletLoss_new_sampling, self).__init__()
        self.margin = margin
        self.sampler = TupleSampler(method=sampling_method)
    ...
    def forward(self, batch, labels, pred):
        """
        Args:
            batch:   torch.Tensor() [(BS x embed_dim)], batch of embeddings
            labels:  np.ndarray [(BS x 1)], for each element of the batch assigns a class [0,...,C-1]
        ...
        """
        # Sample triplets to use for training.
        sampled_triplets = self.sampler.give(batch, labels, pred)
...
class TripletLossCluster(torch.nn.Module):
    def __init__(self, margin=1, sampling_method='random'):
        """
        Basic Triplet Loss as proposed in 'FaceNet: A Unified Embedding for Face Recognition and Clustering'
        Args:
            ...
            sampling_method:    Method to use for sampling training triplets. Used for the TupleSampler-class.
        """
        super(TripletLossCluster, self).__init__()
        self.margin = margin
        self.sampler = TupleSampler(method=sampling_method)

    def triplet_distance(self, anchor, positive, negative, margin_flag=None):
        """
        Compute triplet loss.
        ...
        if margin_flag:
            return torch.nn.functional.relu(
                (anchor - positive).pow(2).sum() - (anchor - negative).pow(
                    2).sum() + self.margin / 2)
        return torch.nn.functional.relu(
            (anchor - positive).pow(2).sum() - (anchor - negative).pow(2).sum() + self.margin)

    def forward(self, batch, cluster_labels, real_label):
        """
        Args:
            batch:   torch.Tensor() [(BS x embed_dim)], batch of embeddings
            labels:  np.ndarray [(BS x 1)], for each element of the batch assigns a class [0,...,C-1]
        ...
        """
        # Sample triplets to use for training.
        sampled_triplets = self.sampler.give(batch, cluster_labels)
        # Compute triplet loss
        loss = []
        for triplet in sampled_triplets:
            if real_label[triplet[0]] == real_label[triplet[2]]:
                loss.append(self.triplet_distance(batch[triplet[0], :], batch[triplet[1], :],
                                                  batch[triplet[2], :], margin_flag=True))
            else:
                loss.append(self.triplet_distance(batch[triplet[0], :], batch[triplet[1], :],
                                                  batch[triplet[2], :], margin_flag=False))
...
### MarginLoss with trainable class separation margin beta. Runs on Mini-batches as well.
class MarginLossCluster(torch.nn.Module):
    def __init__(self, margin=0.2, nu=0, beta=1.2, n_classes=294, beta_constant=False,
                 sampling_method='distance'):
        """
        Basic Margin Loss as proposed in 'Sampling Matters in Deep Embedding Learning'.
        ...
        Returns:
            Nothing!
        """
        super(MarginLossCluster, self).__init__()
        self.margin = margin
        self.n_classes = n_classes
        self.beta_constant = beta_constant

        self.beta_val = beta
        self.beta = beta if beta_constant else torch.nn.Parameter(torch.ones(294) * beta)
    ...
    def forward(self, batch, cluster_labels, real_label):
        ...
        if isinstance(cluster_labels, torch.Tensor): cluster_labels = cluster_labels.detach().cpu().numpy()
        if isinstance(real_label,
                      torch.Tensor): real_label = real_label.detach().cpu().numpy()
        sampled_triplets = self.sampler.give(batch, cluster_labels)
        ...
        #     print(idx,cluster_labels[triplet[0]])
        ...
        if self.beta_constant:
            ...
            beta = torch.stack(
                [self.beta[cluster_labels[triplet[0]]] for triplet in sampled_triplets]).type(
                torch.cuda.FloatTensor)
...
"""================================================================================================="""
class MarginLoss_M(torch.nn.Module):
    def __init__(self, margin=0.2, nu=0, beta=1.2, n_classes=100, beta_constant=False,
                 sampling_method='distance'):
        """
        Basic Margin Loss as proposed in 'Sampling Matters in Deep Embedding Learning'.
        ...
        Returns:
            Nothing!
        """
        super(MarginLoss_M, self).__init__()
        self.margin = margin
        self.n_classes = n_classes
        self.beta_constant = beta_constant
    ...
    def forward(self, batch, labels, pred_label):
        """
        Args:
            batch:   torch.Tensor() [(BS x embed_dim)], batch of embeddings
            labels:  np.ndarray [(BS x 1)], for each element of the batch assigns a class [0,...,C-1]
        ...
        sampled_triplets = self.sampler.give(batch, labels, pred_label)
...
class ProxyNCALoss(torch.nn.Module):
    ...
    def forward(self, batch, labels):
        ...
        """
        # Normalize batch in case it is not normalized
        # (which should never be the case for ProxyNCA, but still).
        # Same for the PROXIES. Note that the multiplication by 3 seems arbitrary,
        # but helps the actual training.
        ...
        neg_proxies = torch.stack(
            [torch.cat([self.all_classes[:class_label], self.all_classes[class_label + 1:]]) for
             class_label in labels])
...
class CEClassLoss(torch.nn.Module):
    ...
    def forward(self, batch, labels):
        ...
        return self.ce_loss(self.mapper(batch), labels.type(torch.cuda.LongTensor)), \
               torch.topk(self.mapper(batch), k=3, dim=1)[1]
